...
pub fn main() anyerror!void {
    ...
    var last_board_to_win: usize = undefined;
    var last_num_called: u32 = undefined;

    for (number_calls.items) |num| {
        log("Number call: {}", .{num});
        ...
        for (boards.items) |*board, board_index| {
            ...
            if (board.hasWon()) {
                ...
                last_num_called = num;
                log("Board {} won with {}", .{ board_index, num });
            }
        }
    }

    var final_sum: u32 = boards.items[last_board_to_win].sumUnmarked();
    final_sum *= last_num_called;
    log("Board {} won with {} ({})", .{ last_board_to_win, last_num_called, final_sum });
}

...
pub fn main() anyerror!void {
    ...
    for (scanners.items) |scanner_a| {
        for (scanners.items) |scanner_b| {
            const diff = Vec3.sub(scanner_a.offset, scanner_b.offset);
            const dist = (try std.math.absInt(diff.x)) + (try std.math.absInt(diff.y)) + (try std.math.absInt(diff.z));
            largest_distance = std.math.max(largest_distance, dist);
        }
    }

    log("Largest distance: {}", .{largest_distance});
}

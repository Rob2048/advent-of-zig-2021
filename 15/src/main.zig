...
pub fn main() anyerror!void {
    ...
    var single_tile_size: usize = 0;
    var single_tile = std.ArrayList(Node).init(allocator);
    defer single_tile.deinit();

    var line_iter = std.mem.split(u8, file.bytes, "\r\n");
    var line_index: usize = 0;
    while (line_iter.next()) |line| {
        if (single_tile_size == 0) {
            single_tile_size = line.len;
        }

        for (line) |char| {
            const risk: u8 = char - '0';

            try single_tile.append(Node{
                .index = single_tile.items.len,
                .risk = risk,
            });
        }
    ...
    assert(single_tile.items.len == single_tile_size * single_tile_size);

    // Copy grid tiles.
    var cave_size: usize = single_tile_size * 5;
    var cave = try std.ArrayList(Node).initCapacity(allocator, single_tile_size * single_tile_size);
    defer cave.deinit();

    var cell_iter: usize = 0;
    while (cell_iter < cave_size * cave_size) : (cell_iter += 1) {
    ...
    while (tile_y < 5) : (tile_y += 1) {
        ...
        while (tile_x < 5) : (tile_x += 1) {
            // log("Copy to tile: {}, {}", .{ tile_x, tile_y });

            var cell_y: usize = 0;
            while (cell_y < single_tile_size) : (cell_y += 1) {
                var cell_x: usize = 0;
                while (cell_x < single_tile_size) : (cell_x += 1) {
                    const src_x = cell_x;
                    const src_y = cell_y;
                    const src_index = src_x + src_y * single_tile_size;

                    const dst_x = tile_x * single_tile_size + cell_x;
                    const dst_y = tile_y * single_tile_size + cell_y;
                    const dst_index = dst_x + dst_y * cave_size;

                    const src_risk = single_tile.items[src_index].risk - 1;
                    const risk_final_value = (src_risk + tile_x + tile_y) % 9;
                    cave.items[dst_index].risk = @intCast(u8, risk_final_value + 1);

                    // log("Src: {},{}({}) Dst: {},{}({})", .{ src_x, src_y, src_index, dst_x, dst_y, dst_index });
                }
            }
        }
    }
    ...
    while (true) {
        ...
        if (x == cave_size - 1 and y == cave_size - 1) {
            ...
            final_cost = active_node.shortest_value;
    ...
    log("Shortest path cost: {}", .{final_cost});
}

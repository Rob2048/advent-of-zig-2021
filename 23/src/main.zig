...
const State = struct {
    ...
    pub fn fromStartCondition(values: []const u8) State {
        ...
        for (values) |value, index| {
            ...
            if (room == cell_type and index % 4 == 3) {
    ...
    pub fn debugPrint(self: State) void {
        ...
        var cells: [23]u8 = undefined;
        for (cells) |*c, index| {
            c.* = @intCast(u8, (self.state >> @intCast(u7, index * 3)) & 0x7);
        }
        ...
        var row_iter: usize = 0;
        while (row_iter < 4) : (row_iter += 1) {
            std.debug.print("    {c}   {c}   {c}   {c}\n", .{
                State.getPrintChar(cells[row_iter + 7]),
                State.getPrintChar(cells[row_iter + 11]),
                State.getPrintChar(cells[row_iter + 15]),
                State.getPrintChar(cells[row_iter + 19]),
            });
        }
    }
    ...
    pub inline fn getCell(self: State, index: usize) u8 {
        return @intCast(u8, (self.state >> @intCast(u7, index * 3)) & 0x7);
    }

    pub inline fn setCell(self: *State, index: usize, value: u8) void {
        var new_state = self.state;

        new_state &= ~(@as(i128, 0x7) << @intCast(u7, index * 3));
        new_state |= @as(i128, value) << @intCast(u7, index * 3);

        self.state = new_state;
    }

    pub inline fn isFinishCondition(self: State) bool {
        const value: i128 = comptime result: {
            var result: i128 = 0;
            var room_iter: usize = 7;
            while (room_iter < 23) : (room_iter += 1) {
...
var state_lookup: std.AutoHashMap(i128, usize) = undefined;
...
pub fn main() anyerror!void {
    ...
    state_lookup = std.AutoHashMap(i128, usize).init(allocator);
    ...
    // var initial_state = State.fromStartCondition(&[_]u8{ 'B', 'D', 'D', 'A', 'C', 'C', 'B', 'D', 'B', 'B', 'A', 'C', 'D', 'A', 'C', 'A' });
    var initial_state = State.fromStartCondition(&[_]u8{ 'C', 'D', 'D', 'B', 'D', 'C', 'B', 'A', 'D', 'B', 'A', 'B', 'A', 'A', 'C', 'C' });
        ...
        const hallwayers_moved = hallwayers: {
            ...
            while (hall_iter < 7) : (hall_iter += 1) {
                ...
                if (cell > 0 and cell < 5) {
                    ...
                    const home0 = active_state.getCell(3 + (cell * 4));
                    const home1 = active_state.getCell(4 + (cell * 4));
                    const home2 = active_state.getCell(5 + (cell * 4));
                    const home3 = active_state.getCell(6 + (cell * 4));

                    if (home3 == 0) {
                        if (try checkMoveAndPushState(&active_state, src, 6 + (cell * 4))) {
                            break :hallwayers true;
                        }
                    } else if (home3 == 5) {
                        if (home2 == 0) {
                            if (try checkMoveAndPushState(&active_state, src, 5 + (cell * 4))) {
                                break :hallwayers true;
                            }
                        } else if (home2 == 5) {
                            if (home1 == 0) {
                                if (try checkMoveAndPushState(&active_state, src, 4 + (cell * 4))) {
                                    break :hallwayers true;
                                }
                            } else if (home1 == 5) {
                                if (home0 == 0) {
                                    if (try checkMoveAndPushState(&active_state, src, 3 + (cell * 4))) {
                                        break :hallwayers true;
                                    }
                                }
                            }
...
fn addWalkTableEntry(cells: []const usize, moves: i64) !void {
    {
        ...
        for (cells[1..]) |cell| {
            entry.mask |= @as(i128, 0x7) << @intCast(u7, (cell * 3));
        }
...
fn generateWalkTable(allocator: std.mem.Allocator) !void {
    ...
    var src_iter: i64 = 0;
    while (src_iter < 23) : (src_iter += 1) {
        var dst_iter: i64 = src_iter + 1;
        while (dst_iter < 23) : (dst_iter += 1) {
            cell_count = 0;
            ...
            if (src_iter < 7 or dst_iter < 7) {
                ...
                const dst_room: i64 = @divTrunc((dst - 7), 4);
                ...
                const src_room: i64 = @divTrunc((src - 7), 4);
                const dst_room: i64 = @divTrunc((dst - 7), 4);
                ...
                var src_exit_iter: i64 = @mod((src - 7), 4);
                while (true) : (src_exit_iter -= 1) {
                    cell_list[cell_count] = @intCast(usize, src_room * 4 + 7 + src_exit_iter);
                    cell_count += 1;
                    move_count += 1;

...
const Node = struct {
    ...
    pub fn clone(self: *Node, parent: ?*Node) anyerror!*Node {
        var new_node = try createNode();

        new_node.parent = parent;

        new_node.left.literal = self.left.literal;
        if (self.left.node) |copy_node| {
            new_node.left.node = try copy_node.clone(new_node);
        }

        new_node.right.literal = self.right.literal;
        if (self.right.node) |copy_node| {
            new_node.right.node = try copy_node.clone(new_node);
        }
...
pub fn main() anyerror!void {
    nodes = try std.ArrayList(Node).initCapacity(std.heap.page_allocator, 1000000);
    defer nodes.deinit();
    ...
    var nums = std.ArrayList(*Node).init(std.heap.page_allocator);
    defer nums.deinit();

    var line_iter = std.mem.split(u8, input_str, "\r\n");
    while (line_iter.next()) |line| {
        var new_num = try createTree(line);
        log("New snail number:", .{});
        new_num.debugPrint();
        try nums.append(new_num);
    }

    log("Total nums: {}", .{nums.items.len});

    var max_magnitude: usize = 0;

    for (nums.items) |first| {
        for (nums.items) |second| {
            if (first == second) {
                continue;
            }

            var first_clone = try first.clone(null);
            var second_clone = try second.clone(null);
            var sum = try addTrees(first_clone, second_clone);

            max_magnitude = std.math.max(max_magnitude, sum.getMagnitude());
        }
    }

    log("Nodes len: {}", .{nodes.items.len});
    log("Magnitude: {}", .{max_magnitude});
}
